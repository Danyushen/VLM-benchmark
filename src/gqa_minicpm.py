...
with open(base_path/'data'/'questions'/'testdev_balanced_questions.json') as f:
    data_questions = json.load(f)

# get the answer from the model for the given question and image
def get_answer(question, image_id):
    image_path = os.path.join(base_path/'data'/'images', image_id + '.jpg')
...
if __name__ == '__main__':
    ...
    with open(base_path/'test_results'/'predictions_testdev_balanced.json', 'w') as f:
        json.dump(predictions_list, f, indent=4)

    # save the wrong predictions as json
    with open(base_path/'test_results'/'wrong_predictions_testdev_balanced.json', 'w') as f:
